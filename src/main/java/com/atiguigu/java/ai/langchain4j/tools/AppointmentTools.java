...
import com.atiguigu.java.ai.langchain4j.service.AppointmentService;
...
//查找数据库中是否包含对应的预约记录
    ...
    @Tool(name = "取消预约", value = "根据参数，查询预约是否存在，如果存在则取消预约，并返回取消结果，否则返回取消失败")
    ...
    @Tool(name = "查询是否有号源", value = "根据科室名称、日期、时间、医生名称，查询是否有号源，并返回查询结果")
    public boolean queryDepartment(
            @P(value = "预约科室名称") String name,
            @P(value = "预约日期") String date,
            @P(value = "时间，可选值：上午/下午") String time,
            @P(value = "预约医生", required = false) String doctorName
    ){
